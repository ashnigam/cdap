...
public class PostgreSqlStructuredTable implements StructuredTable {
  ...
  private String getUnionQuery(Range range, int limit, Collection<Field<?>> filterIndexes,
    ...
    boolean allSameField = filterIndexes.stream()
        .map(Field::getName)
        .distinct()
        .count() <= 1;

    String unionOperator = allSameField ? " UNION ALL " : " UNION ";
    StringJoiner unionJoiner = new StringJoiner(unionOperator);
    for (Field<?> field : filterIndexes) {
      StringBuilder subQuery = new StringBuilder();
      String condition =
          field.getValue() == null ? field.getName() + " IS NULL" : field.getName() + " = ?";
      appendSubQuery(subQuery, tableName, range, limit, condition, fieldsToSort, sortOrder);
      unionJoiner.add("(" + subQuery.toString() + ")");
    }

    query.append(unionJoiner.toString())
        .append(" ")
        .append(getOrderByClause(fieldsToSort, sortOrder))
        .append(" LIMIT ").append(limit).append(";");

    return query.toString();
  }

  private void appendSubQuery(StringBuilder query, String tableName, Range range, int limit,
      String condition,
      Collection<String> fieldsToSort, SortOrder sortOrder) {
